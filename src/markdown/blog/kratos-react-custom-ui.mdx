...
[Add Authentication to your Next.js / React Single Page Application (SPA)](/login-spa-react-nextjs-authentication-example-api-open-source)
...
variable. To get started we recommend to run Ory Kratos in an Ory Cloud Project,
which is free for developers. You can create a new project on
[console.ory.sh](https://www.ory.sh/docs/start-building/create-project) or you
[via the Ory CLI](https://www.ory.sh/docs/guides/cli/installation). Install the
CLI with the package manager of your choice on
[Linux](https://www.ory.sh/docs/guides/cli/installation#linux),
[macOs](https://www.ory.sh/docs/guides/cli/installation#macos), or
[Windows](https://www.ory.sh/docs/guides/cli/installation#windows).
...
[SDK URL](https://www.ory.sh/docs/concepts/services-api) of the project you just
created:
...
You can also run Ory Kratos on your own machine and develop in a local
environment. A quick way to begin is to run the
[Ory Kratos Docker quickstart](https://www.ory.sh/docs/kratos/quickstart) as it
includes all the necessary dependencies. You can run Ory Kratos without Docker
as well!
...
You can also configure this (like all other Ory configuration) directly in the
CLI, for example for the registration UI. Just switch out the flow name to
configure the other UIs:
