...
teaser: |
  Build a social networking to share side projects and explore what your peers are making, using open source software and Ory Kratos for authentication.

seo:
  title: |
    Authentication for React, Graphql, and Kubernetes written in Go with code examples
  description: |
...
students can share their side-projects and explore what their peers are doing
and the authentication is done by Ory Kratos! This is the third part of a series
on our learnings during the development process with a focus on authentication.
Feel free to have a look at the parts about the
...
Ory Kratos often works with so-called
[_flows_](https://www.ory.sh/docs/kratos/self-service). A flow is a process that
may consist of several steps such as account recovery or login with MFA. For
login with username/password this may seem like an overkill since there is only
a singe request (send credentials in exchange for session cookie), but the flow
pattern is used to allow for MFA logins in the future.
...
Let's say you forgot your password and requested a recovery link via mail. The
problem Ory developers needed to solve here is to send some _token_ from the
...
1. User clicks recovery link in the format
   ...
   handled by Ory Kratos independent from the UI!
...
Ory Kratos has a [Helm Chart](https://k8s.ory.sh/helm/kratos.html) that allows
to install it in a cluster more or less conveniently. The interesting part here
is how to configure Ory Kratos (set redirect links, email-credentials, and
more). We do this by providing values to the Helm Chart in our Helmfile. All
this is managed in our
[deployment repo](https://gitlab.lrz.de/projecthub/deployment):
...
The traits property of the SDK identity type is untyped since it depends on the
identity schema, so in our own identity type we wrapped it in a map with a
...
I hope to have given a nice introduction to Ory Kratos! Please
[feel free to comment in the discussion](https://github.com/ory/kratos/discussions/2356)
on what we could do more elegantly! Also I'm interested in yoursolutions for
