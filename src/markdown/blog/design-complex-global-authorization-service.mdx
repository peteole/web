...
username as subject. Finally calls the `writeRequest` API to Ory Keto to store
the newly created relation. The idea is that whenever we create a file, we
...
To check relations we add a helper function to our handler that allows us to do
a check whether a user is the owner of a file:
...
This is the reverse of what we just before when adding the ownership relation.
`checkIsOwner` is looking up if the user is the owner of the file in the
namespace `files` - as defined in the step before. In case the user is the owner
we serve the file, if not there is an error: "You are not the owner of the
file".

We have now implemented functions to track, modify and check ownership of files
and are ready to test the full setup now. Launch the client again
([localhost:9000](http://localhost:9000/)) and add the following (feel free to
use different names)

- Username: Bob
- Filename example.txt
- File Content: bobs-darkest-secrets

Click `POST` and you should get the following result:
...
Now add `example.txt` as the file name and press `GET` to access the file.

One problem with this approach is that we check directly for ownership. You
might want to share the file with someone else and you're the owner of the file
so you're allowed to share it. Someone else might be able read it, but they
might not be allowed to update or share it further. In our setup this is not
solely reflected by the ownership relation. In this case we limit the
flexibility to implement permissions on top of this ownership relation.

What we should do instead is track the relations as they are, but then actually
define the permissions based on relations.
...
This read relation can be defined in a lot of different ways. For example when
you can write to a file you normally also can read it. Another pattern might be
that you get a shared link that allows you to read a file. But you are not the
owner of the file and you are not able to edit it. That's why this abstraction
is important if you are building more complex authorization.

We also need to create a second tuple called "workaround tuple". That's because
you currently have to implement a workaround for "subject set rewrites" in Ory
Keto. It's considered a workaround because we are currently working on
[subject set rewrites](https://github.com/ory/keto/issues/263). Using subject
set rewrites we can define that everyone who is owner also has read access
without effort. In the meantime we can use the workaround tuple which says that
you can read the file, the subject set, and if you are owner of the file. In
this workaround tuple the username does not appear but we define read through
ownership - giving everyone who has ownership of a file the `read` permission.
This gives us flexibility later on, so we will be able to remove that permission
or adjust it. We can implement the `write`/ read` relation in the same way.
...
These are the basic building blocks to design a complex authorization service with Ory Keto. Check out the
[full code example on GitHub](https://github.com/vinckr/keto-example) and the
[Ory Keto documentation](https://www.ory.sh/docs/keto) for more information.

>[Share what you created](https://github.com/ory/keto/discussions/new?category=show-and-tell) on the basis this tutorial with the Ory community.
